---
id: AID-DUO-004
title: Comprendre les droits et préférences dans un Duo
slug: droits-preferences-duo
category: duo-partage
summary: Savoir quelles actions appartiennent au titulaire et lesquelles sont individuelles.
tags: [duo, droits, notifications]
offers: [premium_duo]
roles: [owner, duo_member]
rolesLabel: Titulaire et membre Duo
platforms: [web, mobile]
authState: [public_help, connected_action]
screens: [Mon compte, Notifications, Offres]
objectTypes: [compte, utilisateur]
permissions: Aucun prérequis pour lire l’article
relatedArticles: [AID-NOTIF-003, AID-BILL-006]
seoTitle: Comprendre les droits et préférences dans un Duo | Aide Verebona
metaDescription: Savoir quelles actions appartiennent au titulaire et lesquelles sont individuelles.
canonical: /aide/droits-preferences-duo
indexable: true
lang: fr-FR
synonyms: [duo, droits, notifications, préférences, partage, second utilisateur, membre, invitation]
status: published
---

Le Duo partage les données métier mais pas tous les pouvoirs de gestion. Le titulaire de l’abonnement conserve les fonctions de facturation et d’invitation ; le second utilisateur utilise le contenu partagé selon les droits applicables.

## Détails et cas particuliers

**Offre et paiement** — Seul le titulaire modifie l’offre et accède au portail de facturation.

**Invitation** — Le titulaire gère la deuxième place.

**Notifications** — Chaque utilisateur choisit ses propres préférences, enregistrées par user_id.

**Assistant** — Les conversations doivent être privées par utilisateur dans la cible, même si les biens et documents consultés sont ceux du compte partagé.

**Mot de passe** — Toujours individuel ; ne partagez pas les identifiants du titulaire.
