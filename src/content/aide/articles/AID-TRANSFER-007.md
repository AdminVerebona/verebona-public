---
id: AID-TRANSFER-007
title: Comprendre le mode récupération d’un Duo
slug: recuperation-duo
category: transfert-recuperation
summary: Récupérer des biens vers son espace personnel lorsqu’un Duo passe en mode récupération.
tags: [duo, récupération, impayé]
offers: [premium_duo]
roles: [owner, duo_member]
rolesLabel: Titulaire et membre Duo
platforms: [web, mobile]
authState: [public_help, connected_action]
screens: [Duo > Récupération]
objectTypes: [duo, bien]
permissions: Aucun prérequis pour lire l’article
relatedArticles: [AID-DUO-005, AID-BILL-008]
seoTitle: Comprendre le mode récupération d’un Duo | Aide Verebona
metaDescription: Récupérer des biens vers son espace personnel lorsqu’un Duo passe en mode récupération.
canonical: /aide/recuperation-duo
indexable: true
lang: fr-FR
synonyms: [duo, récupération, impayé, mode, transfert, transmission, envoyer, recevoir, récupérer, export brut]
status: published
---

Lorsqu’un compte Duo est en récupération après un impayé, l’application peut afficher un espace de récupération. Les membres ne disposent plus de l’usage normal du Duo, mais peuvent demander le déplacement de biens vers leur compte personnel.

## Procédure

1. **Ouvrez l’espace de récupération** — La liste des biens du Duo est affichée.
2. **Choisissez un bien disponible** — Un bien déjà en déplacement ou suppression est verrouillé.
3. **Cliquez sur « Récupérer »** — Verebona crée une demande de déplacement vers votre espace personnel.
4. **Suivez le statut** — Le bien indique « Déplacement en cours » jusqu’à résolution.
5. **Régularisez si vous souhaitez restaurer le Duo** — Le titulaire peut utiliser le parcours de paiement prévu.

> **Limites et points d’attention** — Les demandes de suppression de biens sont impossibles en mode récupération.
